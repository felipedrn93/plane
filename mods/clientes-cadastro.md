...
- **Semear o `initialExpression` do filtro não filtra nada.** O `getOrCreateFilter` recebe a expressão inicial e ela alimenta a *UI* do filtro, mas a requisição de work items usa `filters[viewId].richFilters` do store — e o `fetchFilters` sobrescreve esse valor com o que está salvo na view. Por isso as condições da URL precisam ser reaplicadas com `updateFilterExpression` **depois** do `fetchFilters` e **antes** do `fetchIssues`. A chave do SWR inclui as condições, senão trocar o `?client_id=` não refaz a busca.
- **O `ClientDropdown` precisa carregar a lista mesmo fechado.** Quem resolve o nome do cliente selecionado é o `clientMap`; enquanto a busca acontecia só ao abrir o painel, o detalhe da tarefa mostrava "Nenhum" para uma tarefa que tinha cliente. O `useSWR` no dropdown usa a mesma chave das telas de cliente, então o SWR deduplica entre todas as instâncias (inclusive uma por linha da planilha).
